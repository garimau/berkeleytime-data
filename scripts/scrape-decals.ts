...
const getDecals = async (semester: string) => {
  try {
    const response = await fetch(
    ...
    if (!partialDecals) return [];

    const decals = partialDecals.map(async (partialDecal) => {
    ...
    return await Promise.all(decals);
  } catch {
    return [];
  }
};
...
const initialize = async () => {
  ...
  for (const semester of semesters) {
    ...
    const semesterDecals = await getDecals(semester);

    const end = performance.now();

    console.log(
      `Fetched ${semesterDecals.length} decals from ${semester} in ${
        end - start
      }ms.`
    );

    decals.push(...semesterDecals);
  }

  return decals;
};

const decals = await initialize();
console.log(decals.length);
